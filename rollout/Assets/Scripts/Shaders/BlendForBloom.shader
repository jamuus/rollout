Shader "Hidden/BlendForBloom" {
	...
	v2f_mt vertMultiTap( appdata_img v ) {
		...
		o.uv[5] = v.texcoord.xy + _MainTex_TexelSize.xy * half2(1.25,-1.25) * 0.125;	
		o.uv[6] = v.texcoord.xy - _MainTex_TexelSize.xy * half2(1.25,-1.25) * 0.125;	
		o.uv[8] = v.texcoord.xy + _MainTex_TexelSize.xy * half2(1.25,1.25) * 0.125;	
		o.uv[9] = v.texcoord.xy - _MainTex_TexelSize.xy * half2(1.25,1.25) * 0.125;
		o.uv[10] = v.texcoord.xy + _MainTex_TexelSize.xy * half2(-1.25,-1.25) * 0.125;	
		o.uv[11] = v.texcoord.xy - _MainTex_TexelSize.xy * half2(-1.25,-1.25) * 0.125;
		o.uv[12] = v.texcoord.xy + _MainTex_TexelSize.xy * half2(1,1) * 0.125;	
		o.uv[13] = v.texcoord.xy - _MainTex_TexelSize.xy * half2(1,1) * 0.125;
		return o;
	}
	...
	half4 fragMultiTapMax (v2f_mt i) : SV_Target {
		...
		outColor = outColor /4;
		outColor = max(outColor, tex2D(_MainTex, i.uv[4].xy));
		return outColor + (outColor / (1 + Luminance(outColor)));
	}
	...
	half4 fragMultiTapBlur (v2f_mt i) : SV_Target {
		...
		outColor += tex2D(_MainTex, i.uv[8].xy);
		outColor += tex2D(_MainTex, i.uv[9].xy);
		outColor += tex2D(_MainTex, i.uv[10].xy);
		outColor += tex2D(_MainTex, i.uv[11].xy);
		outColor += tex2D(_MainTex, i.uv[12].xy);
		outColor += tex2D(_MainTex, i.uv[13].xy);
		outColor = outColor/14;
		outColor += tex2D(_MainTex, i.uv[4].xy);
		return outColor + (outColor / (1 + Luminance(outColor)));
	}
